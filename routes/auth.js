...
router.post('/google-login', async (req, res) => {
  try {
    const { email, googleId, metadata } = req.body;
    
    console.log('1. Google login isteği alındı:', { email, googleId, metadata });
    
    if (!email || !googleId) {
      throw new Error('Email ve googleId gerekli');
    }

    // Kullanıcıyı Supabase'den kontrol et
    ...
    console.log('2. Kullanıcı arama sonucu:', { existingUser, searchError });
    ...
    if (!existingUser) {
      ...
      console.log('3. Yeni kullanıcı oluşturuluyor:', newUserData);
      ...
      try {
        ...
        if (insertError) {
          console.error('4. Kullanıcı oluşturma hatası:', insertError);
          throw insertError;
        }
        
        console.log('5. Yeni kullanıcı oluşturuldu:', newUser);
        user = newUser;
      } catch (insertError) {
        console.error('6. Insert işlemi hatası:', insertError);
        throw insertError;
      }
    } else {
      console.log('7. Mevcut kullanıcı kullanılıyor:', existingUser);
      user = existingUser;
    }
    ...
    try {
      ...
      console.log('8. Token oluşturuldu');
      ...
      console.error('9. Token oluşturma hatası:', tokenError);
      throw tokenError;
    }
  } catch (error) {
    console.error('10. Genel hata:', {
      name: error.name,
    ...
    res.status(500).json({
      success: false,
      message: 'Google girişi sırasında bir hata oluştu',
